...
import { herroepBetaling, meldBetaling, useArchief, useBetaling } from './data'
import type { Archief, Group, Person } from './data'
import { bedrag, euro, mededeling } from './period'
...
/** The open card + its three copy fields + meld/herroep, for the most recent archief. */
function HuidigeBetaling({
  ...
  archief: Archief & { id: string }
  personRef: string
  nick: string
  group: Group
  onToast: (tekst: string) => void
}) {
  const status = useBetaling(archief.id, personRef)
    ...
    return (
      <div style={{ margin: '20px 14px 0', borderRadius: 12, background: 'rgba(216,246,81,.1)', border: '1px solid rgba(216,246,81,.35)', padding: 16 }}>
        ...
        <div style={{ font: '400 11px/1.5 "Space Grotesk",sans-serif', color: 'rgba(244,241,230,.6)', marginTop: 6 }}>
          Periode {archief.nr} staat afgevinkt.
        </div>
      </div>
    )
  }

  const t = archief.totals[personRef] ?? { streep: 0, bak: 0 }
  const bedragVoorMij = bedrag(t, archief.prijs, archief.bakPrijs)
  ...
  const velden: Veld[] = [
    ...
    { key: 'ref', label: 'MEDEDELING', waarde: mededeling(nick, archief), kopie: mededeling(nick, archief) },
  ]
  ...
  const meldBetaald = async () => {
    await meldBetaling(archief.id, personRef)
    onToast('Doorgegeven aan de drankleider')
  }

  const herroep = async () => {
    await herroepBetaling(archief.id, personRef)
      ...
      <div style={{ margin: '0 14px', borderRadius: 12, background: '#1B1D17', border: `1px solid ${gemeld ? 'rgba(240,163,43,.5)' : 'rgba(228,72,58,.45)'}`, padding: 16 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
          ...
          <span style={{ font: '400 10px ui-monospace,monospace', color: 'rgba(244,241,230,.45)', marginLeft: 'auto' }}>
            {`PERIODE ${archief.nr} · ${archief.start} → ${archief.eind}`.toUpperCase()}
          </span>
        </div>
        ...
        <div style={{ font: '400 11px/1.5 "Space Grotesk",sans-serif', color: 'rgba(244,241,230,.5)', marginTop: 6 }}>
          {t.streep} streepjes · {euro(archief.prijs)} per streepje
        </div>
...
function HistoriekRij({ archief, personRef }: { archief: Archief & { id: string }; personRef: string }) {
  const status = useBetaling(archief.id, personRef)
  const t = archief.totals[personRef] ?? { streep: 0, bak: 0 }
  ...
  return (
    <div style={{ background: '#161811', padding: '13px 18px', display: 'flex', alignItems: 'center', gap: 11 }}>
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ font: '400 16px/1 Anton,sans-serif', color: paper, textTransform: 'uppercase' }}>Periode {archief.nr}</div>
        <div style={{ font: '400 10.5px/1.45 "Space Grotesk",sans-serif', color: 'rgba(244,241,230,.42)', marginTop: 4 }}>
          {archief.start} → {archief.eind} · {t.streep} streepjes
        </div>
      </div>
      <div style={{ textAlign: 'right', flex: 'none' }}>
        <div style={{ font: '500 14px "Space Grotesk",sans-serif', color: paper }}>{euro(bedrag(t, archief.prijs, archief.bakPrijs))}</div>
...
export function Betalen({ user, people, group, onToast }: { user: User; people: Person[]; group: Group; onToast: (tekst: string) => void }) {
  const myRef = 'user:' + user.uid
  const nick = people.find((p) => p.personRef === myRef)?.nick ?? '?'
  const archieven = useArchief()
  const [nieuwste, ...eerdere] = archieven
  ...
  return (
    <div style={{ flex: 1, minHeight: 0, overflowY: 'auto' }}>
      ...
      {nieuwste ? (
        <HuidigeBetaling archief={nieuwste} personRef={myRef} nick={nick} group={group} onToast={onToast} />
      ) : (
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 1, background: 'rgba(244,241,230,.08)' }}>
        {eerdere.map((a) => (
          <HistoriekRij key={a.id} archief={a} personRef={myRef} />
        ))}
      </div>
    </div>
  )
